...
class ConfigManager:
    ...
    def _validate_migration(self, migration: Dict[str, Any], index: int) -> None:
        ...
        try:
            ...
            raise ValueError(f"Migration {index}: invalid regex pattern '{migration['from_pattern']}': {e}")
        ...
        if "effective_date" in migration:
            try:
                ...
                raise ValueError(f"Migration {index}: invalid date format '{migration['effective_date']}': {e}")
